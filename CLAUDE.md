...
All games share one `gameSessions` + `sessionParticipants` pair of tables, not per-game session tables. A session has a `gameType` (`live-quiz` | `backgammon` | `sudoku` | `chess` | `hitster` | `word-links` | `connect-four` | `signal-words` | `bluff-dice` | `squad-surge` — all implemented; word-links and squad-surge are local-only solo play, no Convex session). Squad Surge is also the first fullscreen game: its routes set `staticData: { fullscreen: true }` (typed in `src/lib/router-static-data.ts`), which `src/routes/__root.tsx` reads to hide the Arcade Header/Footer, and its screens render inside the reusable `src/components/games/FullscreenGameShell.tsx`, a `joinMode` (`room` code, `challenge` share-link, or `solo`), and an `authPolicy` (`guestAllowed` | `signedInRequired` | `hostChoice`). Per-game state lives in its own table keyed by `sessionId` (`liveQuizStates`/`liveQuizAnswers`, `backgammonGameStates`/`backgammonMoves`) and is populated/mutated by that game's own Convex module (`convex/quiz.ts`, `convex/backgammon.ts`).
